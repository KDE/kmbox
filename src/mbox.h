...
public:
     ...
     *
     * \sa setLockType( LockType ), unlock()
     */
    [[nodiscard]] bool lock();
     ...
     *
     * Returns true if all offsets refer to a mbox separator line and a file was
    ...
    /*!
     * Reads the entire message from the file for the given mbox \a entry. If the
     ...
     * Returns Message for the given entry or 0 if the file could not be locked
    ...
    /*!
     * Reads the headers of the message for the given mbox \a entry. If the
     * mbox file is not locked this method will lock the file before reading and
     * unlock it after reading. If the file already is locked, it will not
     * unlock the file after reading the entry.
     *
